...
if [[ "$func" == "add_const" ]]; then
    # Read the dataset and constant from the environment variables
    data=$(echo "$DATA" | python3 -c "import json, sys; print(json.load(sys.stdin))")
    const=$CONST

    # Call the python script
    python3 ./add_const.py "$data" "$const" "$kind"
